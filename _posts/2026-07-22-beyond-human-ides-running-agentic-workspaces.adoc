---
title: "Beyond Human IDEs: Running Agentic Workspaces in Eclipse Che"
layout: post
author: Rohan Kumar
description: >-
...
categories: []
keywords: ['Agentic Workspaces', 'CDE', 'Eclipse Che', 'AI', 'Kubernetes', 'ttyd', 'Devfile']
...
| Workspace shape
| Full IDE (for example Che-Code or VS Code) plus extensions, UI, and layouts
...
* Optional: Git credentials configured in Eclipse Che so the agent can push branches and open pull requests on GitHub automatically
...
* Web Terminal: a minimal `ttyd`-based shell in the browser
...
NOTE: Terminal-based coding agents (such as `claude-code` or `opencode`) must already be present in the DevWorkspace container image, or installed or otherwise injected into the workspace before you use them.
...
== How Agentic CDEs Can Be Beneficial for Teams

Agentic CDEs can be beneficial for safer agentic automation: every agent can run in its own workspace, with Kubernetes-enforced boundaries, instead of sharing a developer's laptop or a long-lived shared VM. Using Che for ttyd-driven agent workspaces can build on that foundation in a few practical ways:
